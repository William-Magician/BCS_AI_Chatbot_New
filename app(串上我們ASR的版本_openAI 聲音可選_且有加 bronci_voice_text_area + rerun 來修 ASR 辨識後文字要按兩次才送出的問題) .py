...
with st.sidebar:
    ...
    if st.session_state.voice_mode:
        ...
        st.session_state.asr_provider = st.radio(
            ...
            format_func=lambda x: "瀏覽器語音辨識" if x == "browser" else "自家 ASR",
            horizontal=True,
        )
        ...
        if st.session_state.asr_provider == "bronci":
            ...
            with col_asr_login:
                if st.button("登入 ASR API", use_container_width=True):
                    try:
            ...
            with col_asr_clear:
                if st.button("清除 ASR Token", use_container_width=True):
                    st.session_state.bronci_asr_token = ""
