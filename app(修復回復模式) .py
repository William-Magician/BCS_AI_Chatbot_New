...
# TTS 設定
# =========================================================
BRONCI_TTS_BASE = os.getenv("BRONCI_TTS_BASE", "https://140.113.30.204:8461")
BRONCI_ASR_BASE = os.getenv("BRONCI_ASR_BASE", "https://140.113.30.204:8451")

# 使用者看得到的輸出語言
# 老師要求：中文改成可講中英文，因此標示為「中英文」
TTS_OUTPUT_LANGUAGE_OPTIONS = {
    "中英文": "cmn-TW",
    "台語": "nan-TW",
    "客語（海陸）": "hak-hoi-TW",
    "客語（四縣）": "hak-xi-TW",
...
BRONCI_TTS_SPEAKER_MAP = {
    "cmn-TW": ["cmn-TW-vs2-F01", "cmn-TW-vs2-M01"],
    "nan-TW": ["nan-TW-vs2-M01", "nan-TW-vs2-M02", "nan-TW-vs2-F01", "nan-TW-vs2-F02"],
    "hak-hoi-TW": ["hak-hoi-TW-vs2-M01", "hak-hoi-TW-vs2-F01"],
    "hak-xi-TW": ["hak-xi-TW-vs2-M01", "hak-xi-TW-vs2-F01"],
}

BRONCI_ASR_MODEL_OPTIONS = {
    "國台英客通用模型": "hakka-normal-0909",
    "國台英通用模型": "taigi-0515",
    "台語漢字通用模型": "taigi-bun-0814",
    "台羅通用模型": "taigi-roma-0814",
    "客語華文通用模型": "hakka-0909",
    "客語漢字通用模型": "hakka-bun-1023",
    "客語拼音通用模型": "hakka-pinyin-0115",
}

BRONCI_SPEAKER_LABELS = {
    "cmn-TW-vs2-F01": "中英文女聲（溫和）",
    "cmn-TW-vs2-M01": "中英文男聲（穩重）",

    "nan-TW-vs2-M01": "台語男聲 1",
    "nan-TW-vs2-M02": "台語男聲 2",
    "nan-TW-vs2-F01": "台語女聲 1",
    "nan-TW-vs2-F02": "台語女聲 2",

    "hak-hoi-TW-vs2-M01": "客語海陸男聲",
    "hak-hoi-TW-vs2-F01": "客語海陸女聲",

    "hak-xi-TW-vs2-M01": "客語四縣男聲",
    "hak-xi-TW-vs2-F01": "客語四縣女聲",
}
...
if "pending_tts_audio_mime" not in st.session_state:
    st.session_state.pending_tts_audio_mime = "audio/mp3"

if "reply_with_voice" not in st.session_state:
    st.session_state.reply_with_voice = True

if "tts_provider" not in st.session_state:
    st.session_state.tts_provider = "openai"   # openai | bronci

if "tts_output_lang" not in st.session_state:
    st.session_state.tts_output_lang = "cmn-TW"

if "openai_tts_voice" not in st.session_state:
    st.session_state.openai_tts_voice = "shimmer"

if "bronci_tts_username" not in st.session_state:
    st.session_state.bronci_tts_username = ""

if "bronci_tts_password" not in st.session_state:
    st.session_state.bronci_tts_password = ""

if "bronci_tts_token" not in st.session_state:
    st.session_state.bronci_tts_token = ""

if "tts_model_name" not in st.session_state:
    st.session_state.tts_model_name = "broncitts"

if "tts_language_code" not in st.session_state:
    st.session_state.tts_language_code = "cmn-TW"

if "tts_voice_name" not in st.session_state:
    st.session_state.tts_voice_name = "cmn-TW-vs2-F01"

if "asr_provider" not in st.session_state:
    st.session_state.asr_provider = "browser"   # browser | bronci

if "bronci_asr_username" not in st.session_state:
    st.session_state.bronci_asr_username = ""

if "bronci_asr_password" not in st.session_state:
    st.session_state.bronci_asr_password = ""

if "bronci_asr_token" not in st.session_state:
    st.session_state.bronci_asr_token = ""

if "bronci_asr_model" not in st.session_state:
    st.session_state.bronci_asr_model = "hakka-normal-0909"

if "bronci_voice_text_area" not in st.session_state:
    st.session_state.bronci_voice_text_area = ""

if "bronci_last_audio_hash" not in st.session_state:
    st.session_state.bronci_last_audio_hash = ""

if "bronci_last_transcript" not in st.session_state:
    st.session_state.bronci_last_transcript = ""

if "realtime_backend" not in st.session_state:
    st.session_state.realtime_backend = "openai"  # openai | bronci_pipeline

if "bronci_live_last_audio_hash" not in st.session_state:
    st.session_state.bronci_live_last_audio_hash = ""
...
def reset_to_case_selection():
    ...
    keys_to_clear = [
        ...
        "tts_language_code",
        "tts_voice_name",
        "pending_tts_audio_mime",
        "asr_provider",
        "bronci_asr_username",
        "bronci_asr_password",
        "bronci_asr_token",
        "bronci_asr_model",
        "bronci_voice_text_area",
        "bronci_last_audio_hash",
        "bronci_last_transcript",
    ]
...
def bronci_tts_login(username: str, password: str) -> str:
    url = f"{BRONCI_TTS_BASE}/api/v1/tts/login"
    payload = {
        "username": username,
        "password": password,
        "rememberMe": True,
    ...
    if not token:
        raise ValueError(f"TTS 登入失敗，回傳內容沒有 token：{data}")
    return token

def bronci_asr_login(username: str, password: str) -> str:
    url = f"{BRONCI_ASR_BASE}/api/v1/login"
    payload = {
        "username": username,
        "password": password,
        "rememberMe": True,
    ...
    token = data.get("access_token") or data.get("token") or ""
    if not token:
        raise ValueError(f"ASR 登入失敗，回傳內容沒有 token：{data}")
    return token


def bronci_asr_get_access_info(token: str) -> dict:
    url = f"{BRONCI_ASR_BASE}/api/v1/streaming/transcript/access-info"
    headers = {
    ...
    print("access-info status:", resp.status_code)
    print("access-info text:", resp.text[:500])
    ...
    try:
        ...
        raise ValueError(f"access-info 不是合法 JSON，原始內容：{resp.text[:500]}")
    items = data.get("data", [])
    if not items:
        raise ValueError(f"取得 ASR access-info 失敗：{data}")
    return items[0]
...
def ensure_wav_16k_mono(input_path: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            f"找不到 ffmpeg，可執行檔路徑為：{FFMPEG_PATH}。"
            "請安裝 ffmpeg，或設定環境變數 FFMPEG_PATH。"
        )
...
def bronci_asr_transcribe_wav(wav_path: str, token: str, model_name: str) -> str:
    access_info = bronci_asr_get_access_info(token)
    ws_base_url = access_info["url"]
    ticket = access_info["ticket"]
    ...
    ws_url = (
        ...
        f"&type=file"
        f"&audioFilename=input.wav"
        f"&saveResult=0"
        f"&enableTransient=0"
        f"&modelName={encoded_model}"
    )
    ...
    try:
        while True:
            raw_msg = ws.recv()
            print("ws init raw:", repr(raw_msg))

            if not raw_msg:
                raise ValueError("WebSocket 沒有回任何內容")

            try:
                msg = json.loads(raw_msg)
            except Exception:
                raise ValueError(f"WebSocket 初始回傳不是 JSON：{raw_msg!r}")
            if msg.get("code") == 180:
                break
            if msg.get("code") in (415, 440, 486, 500, 502, 503, 599):
                raise RuntimeError(f"ASR server error: {msg}")

        with open(wav_path, "rb") as f:
            while True:
                chunk = f.read(320 * 1024)
                if not chunk:
                    break
                ws.send(chunk, opcode=websocket.ABNF.OPCODE_BINARY)

        ws.send("EOS", opcode=websocket.ABNF.OPCODE_TEXT)

        while True:
            raw = ws.recv()
            print("ws raw:", repr(raw))
            ...
            try:
                ...
                raise ValueError(f"WebSocket 回傳不是 JSON：{raw!r}")

            code = msg.get("code")
            if code == 200:
                for item in msg.get("result", []) or []:
                    seg = item.get("segment")
                    txt = item.get("transcript", "").strip()
                    fin = item.get("final", 0)

                    if seg is None or not txt:
                        continue

                    if fin == 1:
                        final_segments[seg] = txt
                    else:
                        partial_segments[seg] = txt
                ...
                break
            elif code in (415, 440, 486, 500, 502, 503, 599):
                raise RuntimeError(f"ASR server error: {msg}")
    ...
    ordered_final = [final_segments[k] for k in sorted(final_segments)]
    if ordered_final:
        return "\n".join(ordered_final).strip()

    ordered_partial = [partial_segments[k] for k in sorted(partial_segments)]
    return "\n".join(ordered_partial).strip()

def bronci_tts_synthesize(
    token: str,
    text: str,
    model_name: str,
    language_code: str,
    ...
    url = f"{BRONCI_TTS_BASE}/api/v1/tts/synthesize"
    headers = {
        "Authorization": f"Bearer {token}",
...
def clean_tts_text(text: str) -> str:
    ...
    clean_text = re.sub(r'\{EMOTION:\d\}', '', clean_text)
...
def synthesize_tts(text: str, selected_case: str) -> tuple[bytes, str]:
    """
    回傳 (audio_bytes, mime_type)
    """
    clean_text = clean_tts_text(text)
    if not clean_text:
        raise ValueError("可供 TTS 合成的文字為空")

    provider = st.session_state.tts_provider

    if provider == "bronci":
        if not st.session_state.bronci_tts_token:
            raise ValueError("尚未登入自家 TTS API")

        language_code = st.session_state.tts_output_lang
        available_speakers = BRONCI_TTS_SPEAKER_MAP.get(language_code, [])
        voice_name = st.session_state.tts_voice_name

        if voice_name not in available_speakers and available_speakers:
            voice_name = available_speakers[0]

        audio_bytes = bronci_tts_synthesize(
            token=st.session_state.bronci_tts_token,
            text=clean_text,
    ...
    voice = st.session_state.get("openai_tts_voice", "shimmer")
    ...
    return tts_response.content, "audio/mp3"
...
def build_language_capability_rule() -> str:
    """
    控制病人/家屬被問「聽不聽得懂、會不會說某語言」時的固定回答。
    """
    asr_provider = st.session_state.get("asr_provider", "browser")
    realtime_backend = st.session_state.get("realtime_backend", "openai")
    tts_lang = st.session_state.get("tts_output_lang", "cmn-TW")

    # 自家 ASR：代表可聽懂國語/台語/客語/英文
    if asr_provider == "bronci" or realtime_backend == "bronci_pipeline":
        listen_rule = """
### 語音理解能力規則
如果醫學生問你「你聽得懂台語嗎？」「你聽得懂客語嗎？」「你聽得懂英文嗎？」
或「我說台語/客語/英文你聽得懂嗎？」：
你要回答：你都聽得懂。
例如：「可以，我都聽得懂，台語、客語、英文都可以。」
"""
    else:
        listen_rule = """
### 語音理解能力規則
如果醫學生問你是否聽得懂某種語言，請自然回答，但不要主動宣稱所有語言都聽得懂。
"""
    ...
    if tts_lang == "cmn-TW":
        speak_rule = """
    ### 語音表達能力規則
    如果醫學生問你「你會不會說中文？」或「你會不會說英文？」
    你要回答：會，我會說中文，也會說英文。
    如果醫學生問你「你會不會說台語？」或「你會不會說客語？」
    你要回答：不太會，我主要可以用中文或英文跟你說。
    """

    elif tts_lang == "nan-TW":
        speak_rule = """
    ### 語音表達能力規則
    如果醫學生問你「你會不會說台語？」
    你要回答：會，我會說台語。
    ...
    elif tts_lang in ["hak-hoi-TW", "hak-xi-TW"]:
        speak_rule = """
    ### 語音表達能力規則
    如果醫學生問你「你會不會說客語？」
    你要回答：會，我會說客語。
...
def compose_system_prompt(latest_user_text: str) -> str:
    ...
    # 使用教案專屬的提示詞組合函式
    language_rule = build_language_capability_rule()
    if selected_case == "npc":
        base_prompt = case_compose_system_prompt(
            stage=stage,
            emotion_mode=emotion_mode,
...
with st.sidebar:
    ...
    # 模式說明
    if st.session_state.voice_mode:
        st.info("🎤 即時語音：可選 OpenAI Realtime，或自家 ASR/TTS 串接模式")

        st.session_state.realtime_backend = st.radio(
            "即時語音方式",
            options=["openai", "bronci_pipeline"],
            format_func=lambda x: "OpenAI 即時語音（原本模式）" if x == "openai" else "自家 ASR → OpenAI → TTS",
            horizontal=False,
        )

        if st.session_state.realtime_backend == "openai":
            voice_options = {
                "shimmer": "女聲（柔和）",
                "alloy": "中性聲",
                "echo": "男聲（沉穩）",
            ...
            st.markdown("### 🔐 自家 ASR API 登入")

            st.session_state.bronci_asr_username = st.text_input(
                "ASR Username",
                value=st.session_state.bronci_asr_username,
            )
            st.session_state.bronci_asr_password = st.text_input(
                "ASR Password",
                value=st.session_state.bronci_asr_password,
                type="password",
            )

            col_asr_login, col_asr_clear = st.columns(2)
            with col_asr_login:
                if st.button("登入 ASR API", use_container_width=True, key="voice_bronci_asr_login"):
                    try:
                        token = bronci_asr_login(
                            st.session_state.bronci_asr_username,
                            st.session_state.bronci_asr_password,
                        )
                        st.session_state.bronci_asr_token = token
                        st.success("ASR API 登入成功")
                    except Exception as e:
                        st.error(f"ASR API 登入失敗：{e}")

            with col_asr_clear:
                if st.button("清除 ASR Token", use_container_width=True, key="voice_bronci_asr_clear"):
                    st.session_state.bronci_asr_token = ""
                    st.success("已清除 ASR Token")

            model_labels = list(BRONCI_ASR_MODEL_OPTIONS.keys())
            model_values = list(BRONCI_ASR_MODEL_OPTIONS.values())
            current_idx = (
                model_values.index(st.session_state.bronci_asr_model)
                if st.session_state.bronci_asr_model in model_values else 0
            )
            selected_model_label = st.selectbox(
                "ASR 模型",
                model_labels,
                index=current_idx,
                key="voice_bronci_asr_model_select",
            )
            st.session_state.bronci_asr_model = BRONCI_ASR_MODEL_OPTIONS[selected_model_label]

            if st.session_state.bronci_asr_token:
                st.caption("✅ 已登入自家 ASR API")
            else:
                st.caption("尚未登入自家 ASR API")
    if st.session_state.voice_input_mode:
        st.markdown("### 🎙️ 語音輸入設定")

        st.session_state.asr_provider = st.radio(
            "ASR 引擎",
            options=["browser", "bronci"],
            format_func=lambda x: "瀏覽器語音辨識" if x == "browser" else "自家 ASR（Bronci / SARC）",
            horizontal=True,
        )

        if st.session_state.asr_provider == "bronci":
            st.markdown("### 🔐 自家 ASR API 登入")

            st.session_state.bronci_asr_username = st.text_input(
                "ASR Username",
                value=st.session_state.bronci_asr_username,
                key="voice_input_bronci_asr_username",
            )
            ...
            st.session_state.bronci_asr_password = st.text_input(
                ...
                value=st.session_state.bronci_asr_password,
                type="password",
                key="voice_input_bronci_asr_password",
            )

            if st.button("登入 ASR API", use_container_width=True, key="voice_input_bronci_asr_login"):
                try:
                    token = bronci_asr_login(
                        st.session_state.bronci_asr_username,
                        st.session_state.bronci_asr_password,
            ...
            selected_model_label = st.selectbox(
                ...
                model_labels,
                index=current_idx,
                key="voice_input_bronci_asr_model_select",
            )
            ...
            if st.session_state.bronci_asr_token:
                ...
                st.caption("尚未登入自家 ASR API")
    st.markdown("### 🔊 語音輸出設定")
    st.session_state.reply_with_voice = st.checkbox(
        "AI 回覆自動播放語音",
        value=st.session_state.reply_with_voice,
    )

    st.session_state.tts_provider = st.radio(
        "TTS 引擎",
        options=["openai", "bronci"],
        format_func=lambda x: "OpenAI TTS" if x == "openai" else "自家 TTS",
        horizontal=True,
    )

    if st.session_state.tts_provider == "bronci":
        lang_labels = list(TTS_OUTPUT_LANGUAGE_OPTIONS.keys())
        lang_values = list(TTS_OUTPUT_LANGUAGE_OPTIONS.values())

        current_lang_idx = (
            lang_values.index(st.session_state.tts_output_lang)
            if st.session_state.tts_output_lang in lang_values else 0
        )

        selected_lang_label = st.selectbox("輸出語言", lang_labels, index=current_lang_idx)
        st.session_state.tts_output_lang = TTS_OUTPUT_LANGUAGE_OPTIONS[selected_lang_label]

    if st.session_state.tts_provider == "openai":
        openai_voice_keys = list(OPENAI_TTS_VOICE_LABELS.keys())

        current_openai_voice = st.session_state.get("openai_tts_voice", "shimmer")
        if current_openai_voice not in openai_voice_keys:
            current_openai_voice = "shimmer"

        st.session_state.openai_tts_voice = st.selectbox(
            "OpenAI TTS 聲音",
            openai_voice_keys,
            index=openai_voice_keys.index(current_openai_voice),
            format_func=lambda x: OPENAI_TTS_VOICE_LABELS[x],
        )

    else:
        st.markdown("### 🔐 自家 TTS API 登入")
        st.session_state.bronci_tts_username = st.text_input(
            "TTS Username",
            value=st.session_state.bronci_tts_username,
        )
        st.session_state.bronci_tts_password = st.text_input(
            "TTS Password",
            value=st.session_state.bronci_tts_password,
            type="password",
        )

        col_tts_login, col_tts_clear = st.columns(2)

        with col_tts_login:
            if st.button("登入 TTS API", use_container_width=True):
                try:
                    token = bronci_tts_login(
                        st.session_state.bronci_tts_username,
                        st.session_state.bronci_tts_password,
                    )
                    st.session_state.bronci_tts_token = token
                    st.success("TTS API 登入成功")
                except Exception as e:
                    st.error(f"TTS API 登入失敗：{e}")

        with col_tts_clear:
            if st.button("清除 TTS Token", use_container_width=True):
                st.session_state.bronci_tts_token = ""
                st.success("已清除 TTS Token")

        if st.session_state.bronci_tts_token:
            st.caption("✅ 已登入自家 TTS API")
        else:
            st.caption("尚未登入自家 TTS API")

        st.session_state.tts_language_code = st.session_state.tts_output_lang
        available_speakers = BRONCI_TTS_SPEAKER_MAP.get(st.session_state.tts_language_code, [])

        if not available_speakers:
            available_speakers = ["cmn-TW-vs2-F01"]

        speaker_idx = (
            available_speakers.index(st.session_state.tts_voice_name)
            if st.session_state.tts_voice_name in available_speakers else 0
        )

        st.session_state.tts_voice_name = st.selectbox(
            "聲音角色",
            available_speakers,
            index=speaker_idx,
            format_func=lambda x: BRONCI_SPEAKER_LABELS.get(x, x),
        )
    st.header("⚙️ 功能選單")
...
if st.session_state.voice_mode:
    ...
    if st.session_state.realtime_backend == "openai":
        ...
        st.info("目前模式：OpenAI Realtime 即時語音。點擊「開始對話」後允許使用麥克風，直接說話即可。")
        ...
        if st.session_state.voice_conversation_ended and st.session_state.voice_messages:
            st.success(
                f"✅ 語音對話已結束，共 {len(st.session_state.voice_messages)} 則訊息，"
                f"時長 {st.session_state.voice_duration // 60} 分 {st.session_state.voice_duration % 60} 秒"
            )
        ...
        st.info("目前模式：自家 ASR → OpenAI 對話 → TTS 播放。錄音結束後會自動辨識並送出。")
        # 顯示自家 ASR 即時語音模式的對話歷史
        ...
        audio_file = st.audio_input(
            "按下麥克風錄音，停止後會自動辨識並送給 AI",
            key="bronci_live_audio_input",
        )
        if audio_file is not None:
            if not st.session_state.bronci_asr_token:
                st.error("請先在側邊欄登入自家 ASR API")
            else:
                raw_path = None
                wav_path = None
                ...
                try:
                    ...
                    if audio_hash != st.session_state.bronci_live_last_audio_hash:
                        st.session_state.bronci_live_last_audio_hash = audio_hash
                        ...
                        wav_path = ensure_wav_16k_mono(raw_path)

                        with st.spinner("ASR 辨識中..."):
                            transcript = bronci_asr_transcribe_wav(
                                wav_path=wav_path,
                                token=st.session_state.bronci_asr_token,
                                model_name=st.session_state.bronci_asr_model,
                            )

                        transcript = transcript.strip()

                        if not transcript:
                            st.warning("ASR 沒有辨識到文字")
                        else:
                            st.success(f"辨識結果：{transcript}")
                            ...
                            if detect_diagnosis_disclosure(transcript):
                                st.session_state.diagnosis_disclosed = True
                            update_stage(transcript)
                            ...
                            with st.chat_message("assistant", avatar=AVATAR_PATIENT):
                                with st.spinner(f"{ROLE_LABEL}思考回覆中..."):
                                    system_prompt = compose_system_prompt(transcript)
                                    ...
                                    if st.session_state.reply_with_voice:
                                        try:
                                            audio_bytes, audio_mime = synthesize_tts(content, selected_case)
                                            st.session_state.pending_tts_audio = base64.b64encode(audio_bytes).decode("utf-8")
                                            st.session_state.pending_tts_audio_mime = audio_mime
        ...
        with st.spinner("正在儲存記錄並上傳到 Google Drive..."):
            try:
                ...
                if result.get("drive_file_id"):
                    ...
                    st.session_state.logged_this_session = False
                    st.warning(f"⚠️ Google Drive 上傳狀態不明：{result}")
...
if not st.session_state.voice_mode:
    if st.session_state.voice_input_mode:
        ...
        if st.session_state.asr_provider == "browser":
            st.caption("目前使用：瀏覽器語音辨識")
            prompt = None
            ...
            <div class="voice-input-container" id="voiceContainer">
                <div id="supportedUI">
                    ...
                    <textarea id="textEditor" class="text-editor"
                              placeholder="語音辨識結果會顯示在這裡，您可以修改後點擊「複製文字」..."
                              rows="2"></textarea>
                ...
                if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
                    ...
                    recognition.onend = function() {
                        ...
                        if (textEditor.value.trim()) {
                            ...
                            statusText.textContent = '✓ 辨識完成！可修改文字後點擊複製';
                        } else {
            ...
            st.caption("目前使用：自家 ASR（Bronci / SARC）")
            prompt = None

            st.markdown("#### 🎤 錄音送自家 ASR")
            st.caption("按下開始錄音，停止後會自動做辨識。")

            audio_file = st.audio_input(
                "請錄音",
                key="bronci_audio_input",
                label_visibility="collapsed",
            )

            if audio_file is not None:
                if not st.session_state.bronci_asr_token:
                    st.error("請先登入 ASR API")
                else:
                    raw_path = None
                    wav_path = None
                    try:
                        ...
                        # 避免同一段錄音在每次 rerun 都重跑辨識
                        if audio_hash != st.session_state.bronci_last_audio_hash:
                            suffix = Path(audio_file.name).suffix.lower() or ".wav"
                            ...
                            st.session_state.bronci_last_audio_hash = audio_hash
                            st.session_state.bronci_last_transcript = transcript
                            st.session_state.voice_input_text = transcript
                            st.session_state.bronci_voice_text_area = transcript
                            st.success("辨識完成")

                    except Exception as e:
                        st.error(f"ASR 辨識失敗：{e}")
                    finally:
                        for p in [raw_path, wav_path]:
                            if p and os.path.exists(p):
                                try:
            ...
            with st.form(key="bronci_voice_input_form", clear_on_submit=False):
                st.text_area(
                    "辨識結果",
                    placeholder="錄音辨識完成後，文字會出現在這裡",
                    height=120,
                    key="bronci_voice_text_area",
                )
                submit_button = st.form_submit_button(
                    "✅ 送出訊息",
                    use_container_width=True,
            ...
            if submit_button and st.session_state.bronci_voice_text_area.strip():
                prompt = st.session_state.bronci_voice_text_area.strip()
...
if st.session_state.get("pending_tts_audio"):
    mime = st.session_state.get("pending_tts_audio_mime", "audio/mp3")
    audio_html = f"""
    <audio autoplay>
        <source src="data:{mime};base64,{st.session_state.pending_tts_audio}" type="{mime}">
    </audio>
    """
