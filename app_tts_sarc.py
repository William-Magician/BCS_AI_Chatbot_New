...
with st.sidebar:
    ...
    if st.session_state.bronci_tts_token:
        st.caption("✅ 已登入 TTS API")
    else:
        st.caption("尚未登入 TTS API")
    ...
    lang_labels = list(BRONCI_TTS_LANG_OPTIONS.keys())
    lang_values = list(BRONCI_TTS_LANG_OPTIONS.values())

    current_lang_idx = (
        lang_values.index(st.session_state.tts_language_code)
        if st.session_state.tts_language_code in lang_values else 0
    )
    ...
    st.session_state.tts_language_code = BRONCI_TTS_LANG_OPTIONS[selected_lang_label]
        ...
        with st.chat_message("assistant", avatar=AVATAR_PATIENT):
            with st.spinner(f"{ROLE_LABEL}思考回覆中..."):
                try:
                    ...
                    # 不管文字模式或語音輸入模式，都可以選擇是否播放語音
                    if st.session_state.reply_with_voice:
                        try:
                            if not st.session_state.bronci_tts_token:
                                raise ValueError("尚未登入 TTS API")

                            clean_text = clean_tts_text(content)
                            if not clean_text:
                                raise ValueError("可供合成的文字為空")

                            wav_bytes = bronci_tts_synthesize(
                            ...
                            st.session_state.pending_tts_audio = base64.b64encode(wav_bytes).decode("utf-8")
...
if st.session_state.get("pending_tts_audio"):
    audio_html = f"""
    <audio autoplay>
        <source src="data:audio/wav;base64,{st.session_state.pending_tts_audio}" type="audio/wav">
    </audio>
    """
